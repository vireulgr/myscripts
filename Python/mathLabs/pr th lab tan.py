...
class MyApp:
    def __init__(self, parent ):
                ...
                # description frame:
                self.dscr_frame = Frame( self.myContainer1, width=300, height=150, bg = "tan" )#,# relief=RIDGE, borderwidth=5, \
                                        #labelanchor = "nw", text = u"Описание" )
                self.dscr_frame.pack(side=TOP, fill=BOTH, expand=YES )


                # bottom frame 
                self.btm_frame = Frame( self.myContainer1, bg = "tan" )
                self.btm_frame.pack( side=TOP, fill=BOTH, expand=YES )
                
                # parametres frame
                self.params_frame = Frame( self.btm_frame, width=300, height=250, bg = "tan", relief=RIDGE, borderwidth=5 ) #, \
                                        #labelanchor="nw", text=u"Параметры" )
                self.params_frame.pack( side=LEFT, fill=BOTH, expand=YES )

       
                 # buttons frame
                self.btn_frame = Frame( self.btm_frame, bg="tan" )
                self.btn_frame.pack( side=RIGHT, ipadx = frm_ipadx, ipady = frm_ipady, \
                                padx = frm_padx, pady = frm_pady)
                 # 2nd results frame 
                self.res2_frm = Frame( self.myContainer1, height = 50, relief = RIDGE, borderwidth = 5, background = "tan", \
                                   )
                self.res2_frm.pack( side=LEFT, fill=BOTH, expand=YES )
                
               
         #results frame
                self.results_frm = Frame( self.myContainer1, height = 50, relief = RIDGE, borderwidth = 5, pady=4, padx=4, background = "tan" )#, \
...
Гипотеза - выборка имеет экспоненциальное распределение"
                self.l_dscr = Label( self.dscr_frame, text= description, bg="tan" )
                self.l_dscr.pack() # greek lambda unicode U+03bb

                # widgets for PARAMETERS frame
                self.l_lambd = Label( self.params_frame, text=u"\u03bb := ", bg="tan" )
                ...
                self.e_hyp_param.insert('end', '0.1')
                
                self.l_result = Label( self.params_frame, bg="tan" )
                self.l_result.grid( row=0, column=5, ipadx=8, rowspan=2 )

                # button for BUTTONS frame
                self.btn_launch = Button( self.btn_frame, command = self.btnLaunchClick, bg="tan" )
                self.btn_launch.configure( text=u"Старт", width = self.button_width, padx = btn_padx, pady = btn_pady )
                ...
                self.lb_draw = Listbox( self.results_frm, exportselection=0, 
                                ...
                                yscrollcommand=self.yScroll.set, height=8, width=18 , bg="tan")  #, activestyle="dotbox" 
...
Модуль их разности\nВыборочн. медиана\nРазмах выборки", bg="tan" )
                ...
                self.lb_hyp = Listbox( self.results_frm, exportselection = 0,
                                       ...
                                       yscroll=self.hypScroll.set, bg="tan" )
                ...
                ## Отсюда начинается основная работа программы                           
...
##                        self.btn_launch["background"] = "green"
                ...
                # Генерация последовательности случайных чисел, равномерно
                # распределённых на (0, 1) в количестве self.qty штук
                self.unfrm = np.random.uniform( 0.0, 1.0, self.qty )

                # Применение преобразование G(u)
                self.unfrm = np.log(self.unfrm)
                self.unfrm = np.divide( self.unfrm, -self.lmbd ) 
                ...
                n, bins, patches = ax.hist(self.unfrm, self.qty, normed=1, facecolor='tan')
                ...
                # отрисовка теоретической функции плотности распределения
                ...
                l = ax.plot(self.absciss, self.ordinate, 'k--', linewidth=1)
                
                #r = ax.plot( bins, np.ones_like(bins), 'go' )
                #q = ax.plot( n, n, 'b^')#np.zeros_like(n), 'b^')

                # сортировка массива с результатами испытаний
                self.unfrm.sort()
                #print( self.unfrm )

                # отображение результатов испытаний в ListBox на форме
                self.expDrawString = ""
                ...
                # Очистка списка характеристик с. в.
                ...
                # вычисление теоретической плотности распределения в серединах интервалов
                ...
                # Заполнение данных для 3й части работы
                # заполнение таблицы теоретических вероятностей
...
root.title( u"Чупрова Наталья, Вариант 6")
root.mainloop()
