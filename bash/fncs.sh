...
function install_rpm
{
    ...
    if [ -z "$RPM" ]; then
        ...
        rpm -ihv "$RPM" >$LOG_FILE
        if [ $? -ne '0' ]; then
            #echo "Warning! can't install stats_collector rpm"
            STATUS='2'
        fi
        for remote_ip in $REMOTE_IPS; do
            ssh "root@${remote_ip}" "mkdir -p $G_RPMS_DIR " >$LOG_FILE
            scp "$RPM" "${remote_ip}:$G_RPMS_DIR" >$LOG_FILE
            ssh "root@${remote_ip}" "cd $G_RPMS_DIR && rpm -ihv ${RPM##*/}" >$LOG_FILE
        done
    fi
    return $STATUS
}
